...
double Source::hrate(const std::array<int, 3>& ijk, SourceComponent input) const
{
    auto it = m_cells.find(ijk);
    if (it != m_cells.end()) {
        const auto it2 = std::find_if(it->second.begin(), it->second.end(),
        ...
        return (it2 != it->second.end() && it2->hrate.has_value()) ? it2->hrate.value() : 0.0;
    }
    else {
        return 0.0;
    }
...
// bool Source::hasHrate(const std::array<int, 3>, SourceComponent input) const
// {
...
// bool Source::hasTemperature(const std::array<int, 3>, SourceComponent input) const
// {
//     const auto it = std::find_if(m_cells.begin(), m_cells.end(),
//                                  [&input](const auto& source)
//                                  {
...
double Source::temperature(const std::array<int, 3>& ijk, SourceComponent input) const
{
    auto it = m_cells.find(ijk);
    if (it != m_cells.end()) {
        const auto it2 = std::find_if(it->second.begin(), it->second.end(),
        ...
        return (it2 != it->second.end() && it2->temperature.has_value()) ? it2->temperature.value() : 0.0;
    }
    else {
        return 0.0;
    }
